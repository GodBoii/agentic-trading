 ...
 *
 * All trig-derived coordinates are rounded to 3 decimal places so server
 * and client floating-point paths produce identical strings (no hydration
 * warnings about a 15th-decimal-place mismatch).
 */
...
export default function AICore() {
  const agents = [
    { id: "market", label: "Market", angle: 0, color: "#00D4FF", size: 44 },
    { id: "risk", label: "Risk", angle: 72, color: "#00FF88", size: 38 },
    { id: "execution", label: "Execution", angle: 144, color: "#A78BFA", size: 46 },
    { id: "research", label: "Research", angle: 216, color: "#F472B6", size: 40 },
    { id: "portfolio", label: "Portfolio", angle: 288, color: "#FFB800", size: 42 },
  ];
  ...
  // Pre-compute agent positions deterministically
  const agentPositions = agents.map((a) => {
    const rad = (a.angle * Math.PI) / 180;
    return {
      ...a,
      xPct: r(50 + (Math.cos(rad) * ORBIT) / 6),
      yPct: r(50 + (Math.sin(rad) * ORBIT) / 6),
      beamX: r(Math.cos(rad) * ORBIT),
      beamY: r(Math.sin(rad) * ORBIT),
    };
  });

  // Pre-compute orbiter positions
  const orbiterPositions = innerOrbiters.map((o) => {
    const rad = (o.angle * Math.PI) / 180;
    return {
      ...o,
      cx: r(Math.cos(rad) * o.dist),
      cy: r(Math.sin(rad) * o.dist),
    };
  });
