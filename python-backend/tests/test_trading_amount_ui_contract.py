...
class TradingAmountUIContractTests(unittest.TestCase):
    def test_amount_only_ui_has_save_and_no_manual_start_controls(self):
        source = (Path(__file__).parents[2] / "components" / "agent" / "capital-control.tsx").read_text(encoding="utf-8")
        self.assertIn('aria-label="Margin per trade in rupees"', source)
        self.assertIn("Auto splits account capital", source)
        self.assertIn("tradeSlotLimit", source)
        self.assertIn("Margin per trade", source)
        self.assertIn("fixedSlotCount", source)
        self.assertIn("Concurrent positions", source)
