...
class IntraFinderHealthHandlerTests(TestCase):
    @staticmethod
    def _handler(*, write_error=None, headers_error=None):
        handler = object.__new__(IntraFinderHealthHandler)
        handler.path = "/health"
        handler.service = SimpleNamespace(
            health_payload=lambda: (False, {"status": "unhealthy"})
        )
    ...
    def test_returns_unhealthy_payload(self):
        ...
        handler.wfile.write.assert_called_once_with(b'{"status":"unhealthy"}')
