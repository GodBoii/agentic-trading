...
class StockAgent:
    ...
    def analyze(
        ...
        agent = Agent(
            ...
            instructions=[
                ...
                "The attached image order is: current-day 1m, current-day 5m, current-day 15m, previous-session 15m, daily 1D historical context, volume and participation, momentum and volatility, OHLCV-derived price-structure liquidity, then current/previous TPO market profile.",
                "The initial snapshot supplies identity, time, market state, technical readings, account state, and risk budget. Newer market observations returned by execution tools supersede its older prices and candles. Do not look for read-only tools.",
                ...
                "Choose BUY, SELL, or no trade independently from the charts and observed market data.",
                "There is no tool-call budget. When a preflight check rejects an entry, use the returned current market state to reassess, resize, and retry if a sound trade still exists. A rejection is not directional market evidence.",
    ...
    def _build_prompt(self, stock_packet: Dict[str, Any]) -> str:
        ...
        lines = [
            ...
            "Attached charts in order: current 1m, current 5m, current 15m, previous-session 15m, daily 1D historical context, volume/participation, momentum/volatility, OHLCV-derived price-structure liquidity, and current/previous TPO market profile.",
            "",
